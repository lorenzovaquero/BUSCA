...
from busca.reid.utils import Sequential
from busca.reid.utils import weights_init_kaiming, weights_init_classifier
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, neck, num_classes=1000,
        ...
        if self.neck:
            ...
            if self.distractor_bce:
                self.fc_person = nn.Linear(int((512 * block.expansion)/red), 1,
                                bias=False)
                self.fc_person.apply(weights_init_classifier)
                self.sig = nn.Sigmoid()
        else:
            self.fc = nn.Linear(int((512 * block.expansion)/red), num_classes)
            if self.distractor_bce:
    ...
    def _forward(self, x, output_option='norm', val=False):
        ...
        if self.use_checkpointing:
            last_feature_map = checkpoint.checkpoint(self.layer4, x, val)
        else:
            last_feature_map = self.layer4(x, val)
         
        x = self.avgpool(last_feature_map)
