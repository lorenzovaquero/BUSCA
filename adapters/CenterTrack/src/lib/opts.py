...
class opts(object):
  def __init__(self):
    ...
    # To override the default output_dir
    self.parser.add_argument("--output-dir", default=None, type=str, help="output directory (default is 'YOLOX_outputs')")
    ...
    self.parser.add_argument("--busca-config", default="config/CenterTrack/MOT17/config_centertrack_mot17.yml", type=str, help="config file for BUSCA")
...
#       opt.new_thresh = max(opt.track_thresh, opt.new_thresh)
    ...
    if opt.use_busca:
      ...
      args = merge_args(tracker_args, opt)
      opt = args
    # ^^^ BUSCA args ^^^
