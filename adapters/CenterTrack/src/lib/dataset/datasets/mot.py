...
_BASE_FOLDER = os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', '..', '..', '..')
...
class MOT(GenericDataset):
  ...
  def __init__(self, opt, split):
    ...
    elif opt.dataset_version == '20trainval':
      ann_file = '{}.json'.format('train')
    elif opt.dataset_version == '20test':
  ...
  def run_eval(self, results, save_dir):
    ...
    gt_type_str = '--gt_type {}'.format(gt_type_str) if gt_type_str != '' else \
      ''
    os.system('python {}/src/tools/eval_motchallenge.py '.format(_BASE_FOLDER) + \
              '{}/data/mot{}/{}/ '.format(_BASE_FOLDER, self.year, 'train') + \
