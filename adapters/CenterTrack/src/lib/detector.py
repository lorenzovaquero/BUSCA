...
class Detector(object):
  ...
  def run(self, image_or_path_or_tensor, meta={}):
    ...
    if self.opt.tracking:
      ...
      if hasattr(self.tracker, 'use_busca') and self.tracker.use_busca:
        # vvv *BUSCA* vvv
        img_info = [meta['height'], meta['width'], None, None, [None]]  # img_h, img_w, frame_id, video_id, img_file_name
        display_frame = image_or_path_or_tensor['image'][0].numpy()
        img_size = [meta['height'], meta['width']]
        current_frame = image_or_path_or_tensor['image'][0].numpy()
        results = self.tracker.step(results=results, public_det=public_det, img_info=img_info, img_size=img_size, current_frame=current_frame)
        # ^^^ *BUSCA* ^^^
      else:
