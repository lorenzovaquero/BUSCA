...
def main(exp, args, num_gpu):
    ...
    *_, summary = evaluator.evaluate(
    ...
    if args.mot20:
        evaluator_base_path = os.path.join(val_loader.dataset.data_dir, val_loader.dataset.name)
        gtfiles = glob.glob(os.path.join(evaluator_base_path, '*/gt/gt{}.txt'.format(gt_type)))
    else:
        evaluator_base_path = os.path.join(val_loader.dataset.data_dir, val_loader.dataset.name)
        gtfiles = glob.glob(os.path.join(evaluator_base_path, '*/gt/gt{}.txt'.format(gt_type)))
