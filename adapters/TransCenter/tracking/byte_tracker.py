...
class BYTETracker(object):
    ...
    def get_transcenterdetr_pos(self, use_detector_coordinates=True, omit_unconfirmed=False, omit_lost=False):
        """"Outputs tlbr for each track on the last frame, using the detector's coordinates"""
        tracked_stracks = self.tracked_stracks
        lost_stracks = self.lost_stracks

        relevant_stracks = None
        if omit_unconfirmed:
            unconfirmed = []
            tracked_stracks = []  # type: list[STrack]
            for track in self.tracked_stracks:
        ...
        if omit_lost:
            relevant_stracks = tracked_stracks
        
        else:
            relevant_stracks = joint_stracks(tracked_stracks, lost_stracks)

        pos_list = []
        for track in relevant_stracks:
            pos = track.tlbr.copy()
            if use_detector_coordinates:
                pos = pos * track.scale
            pos_list.append(pos)

        if len(pos_list) > 0:
            pos_list = torch.from_numpy(np.array(pos_list)).to('cuda').float()
        else:
            pos_list = None
